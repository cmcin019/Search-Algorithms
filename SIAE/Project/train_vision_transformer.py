...
from timm.models.vision_transformer import vit_base_patch16_224 as vit_16
	...
	for images, labels in tqdm(loader):
		...
		with torch.no_grad():
			...
			if plot:
				...
				fig_s.savefig(f'images/vit/{pgd.__name__}/fig_{it}_original.jpg', bbox_inches='tight', dpi=150)
				
				np_images = torchvision.utils.make_grid(images.cpu().data, normalize=True).numpy()
				fig, ax = plt.subplots()
				ax.imshow(np.transpose(np_images,(1,2,0)))
				fig.savefig(f'images/vit/{pgd.__name__}/fig_{it}_attack.jpg', bbox_inches='tight', dpi=150)
				...
				fig_acc.savefig(f'images/vit/{pgd.__name__}/Accuracy_{it}.jpg', bbox_inches='tight', dpi=150)
			...
			if pgd != None:
				...
				fp = open(f'images/vit/{pgd.__name__}/fig_{it}_log.txt', 'w')
...
def experiment(original_model, model, pgd, plot=False, pgd_per=[]):
	acc_before = model_accuracy(original_model, pgd=pgd, pgd_per=pgd_per, plot=True)
	print(acc_before)
	acc_attack = None
...
def run(model, plot, spgd, attacks=[inv_boundary_attack], epsilons=[0, .1, .2, .3, .45], steps=1, alpha=0.01):
	original_model = vit(img_size=28,in_chans=1,num_classes=10).to(device=device)
	original_model.load_state_dict(torch.load('models/' + dataset + '/vit.pt'))
	acc = []
	for alg in attacks:
		for eps in epsilons:
			if spgd:
				if device=='cuda:0':
					model.to(device='cpu')
				model = vit(img_size=28,in_chans=1,num_classes=10).to(device=device)
			else:
				model.load_state_dict(torch.load('models/' + dataset + '/vit.pt'))
...
def main() -> None :
	...
	model = vit(img_size=28,in_chans=1, num_classes=10)
	if not load:
		...
		for acc in range(len(acc_list)):
			...
			torch.save(model.state_dict(), 'models/' + dataset + '/vit.pt')
	else:
		model = vit(img_size=28,in_chans=1, num_classes=10)
		model.load_state_dict(torch.load('models/' + dataset + '/vit.pt'))
		model.to(device=device)
...
if __name__ == "__main__":
	...
	oracle = cnn(in_chans=1, num_classes=10)
	oracle.load_state_dict(torch.load('models/' + dataset + '/resnet.pt'))
	oracle.to(device=device)
	oracle.eval()

	brother = vit_16(img_size=28,in_chans=1, num_classes=10)
	brother.load_state_dict(torch.load('models/' + dataset + '/vit_II.pt'))
