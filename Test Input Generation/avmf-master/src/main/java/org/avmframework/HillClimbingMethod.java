...
public class HillClimbingMethod {
   ...
   */
  protected void hillClimbingSearch(AbstractVector abstractVector)
    ...
    while (nonImprovement < abstractVector.size()) {
      ...
      while (variableIndex < abstractVector.size() && nonImprovement < abstractVector.size()) {
        ...
        // Search is only done using the tweak function
        variableSearch(var);
        ...
        if (current.betterThan(lastImprovement)) {
          ...
          // break when first improvement is found
          break;
        } else {
          nonImprovement++;
        }
